...
class UsersSelector
{
     ...
     * @return    void
     */
    function ListAllUsers($channel=0)
    {
        ...
        $this->dsql->SetQuery("SELECT ad.id,ad.uname,ad.department FROM `#@__admin` ad LEFT JOIN dede_sys_enum en on ad.department=en.ename WHERE ad.id != 1 ORDER BY en.disorder ");
        ...
        while($row=$this->dsql->GetObject(0))
        {
            ...
            $ischeck = in_array($userid, $oldvalues) ? ' checked' : '';
            $chackRadio = "<input style='float:left;width: 18px;margin: 6px 5px;' type='checkbox' name='seluserid' value='{$userid}' $ischeck > <span style='float:left;width: 150px'>名称：{$uname}</span>";
            $chackRadio .= "<span id='dpt' style='float:left;'  style='margin-left: 20px'>单位：{$department}</span>";
            $soncat = '';

            if($chackRadio=='' && $soncat=='') continue;
            echo "<div class='quickselItem' onclick='SelectQianShouRen();' onmouseover='QianShouRenOver();' onmouseout='QianShouRenOut();'>\r\n";
            echo "    <div class='topcat'>{$chackRadio}</div>\r\n";
