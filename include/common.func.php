...
//得到值班表的文章标题的日期（今日0-8点为昨日，其它日期不变）
function GetOnDutyDate($querydate)
{

    $curr_time = time();
    $curr_date = date("Y-m-d",time());
    $timeBegin1 = strtotime($curr_date."0:00".":00");
    $timeEnd1 = strtotime($curr_date."23:59".":59");

    $day=$querydate;
    if($querydate >= $timeBegin1 && $querydate <= $timeEnd1){
        $timeEnd1 = strtotime($curr_date."8:00".":00");
...
//在值班表中与某日字条串比较
function GetCompareDateResult($zhibanbiaodate,$date){
    $date1=date("Y年m月d日",$date);
    $date2=date("Y/m/d",$date);
    $date3=date("Y-m-d",$date);
    $date4=date("Y.m.d",$date);
    $date5=date("Y年n月j日",$date);
    $date6=date("Y/n/j",$date);
    $date7=date("Y-n-j",$date);
    $date8=date("Y.n.j",$date);
    $date9=date("n月j日",$date);
    $date10=date("n/j",$date);
    $date11=date("n-j",$date);
    $date12=date("n.j",$date);
    $date13=date("m月d日",$date);
    $date14=date("m/d",$date);
    $date15=date("m-d",$date);
    $date16=date("m.d",$date);
    $pos2 = strcasecmp($zhibanbiaodate, $date1)&&strcasecmp($zhibanbiaodate, $date2)&&strcasecmp($zhibanbiaodate, $date3)&&strcasecmp($zhibanbiaodate, $date4)&&
        strcasecmp($zhibanbiaodate, $date5)&&strcasecmp($zhibanbiaodate, $date6)&&strcasecmp($zhibanbiaodate, $date7)&&strcasecmp($zhibanbiaodate, $date8)&&
        strcasecmp($zhibanbiaodate, $date9)&&strcasecmp($zhibanbiaodate, $date10)&&strcasecmp($zhibanbiaodate, $date11)&&strcasecmp($zhibanbiaodate, $date12)&&
        strcasecmp($zhibanbiaodate, $date13)&&strcasecmp($zhibanbiaodate, $date14)&&strcasecmp($zhibanbiaodate, $date15)&&strcasecmp($zhibanbiaodate, $date16) ;
    return $pos2;
...
function ShowMsg($msg, $gourl, $onlymsg=0, $limittime=0)
{
    ...
    else
    {
      ...
      function JumpUrl(){
        ...
        $rmsg .= "<div style='padding:6px;font-size:12px;border-bottom:1px solid #DADADA;background:#DBEEBD url({$GLOBALS['cfg_plus_dir']}/img/wbg.gif)';'><b>西山分局 提示信息！</b></div>\");\r\n";
