...
//    $ipallowseg=preg_split('//', $ipallowseg, -1, PREG_SPLIT_NO_EMPTY);
//    $ipallowsegmask=explode(";",$ipallowsegmask);
//    $ipdenieseg=explode(";",$ipdenieseg);
//    $ipdeniesegmask=explode(";",$ipdeniesegmask);
    $ipallowsegtest=preg_split('/;/', $ipallowseg, -1, PREG_SPLIT_NO_EMPTY);
    $ipallowsegmasktest=preg_split('/;/', $ipallowsegmask, -1, PREG_SPLIT_NO_EMPTY);
    $ipdeniesegtest=preg_split('/;/', $ipdenieseg, -1, PREG_SPLIT_NO_EMPTY);
    $ipdeniesegmasktest=preg_split('/;/', $ipdeniesegmask, -1, PREG_SPLIT_NO_EMPTY);

//    array_filter($ipallowseg);
//    array_filter($ipallowsegmask);
//    array_filter($ipdenieseg);
//    array_filter($ipdeniesegmask);
    if( count($ipallowsegtest)!= count($ipallowsegmasktest) ||  count($ipdeniesegtest)!= count($ipdeniesegmasktest)){
        ShowMsg("ip 地址或子网掩码需要同时设置。",-1);
        exit;
    }
    for ($i=0;$i<count($ipallowsegtest);$i++){

        if(!(filter_var($ipallowsegtest[$i], FILTER_VALIDATE_IP) &&(filter_var($ipallowsegmasktest[$i], FILTER_VALIDATE_IP))))
         {
            ShowMsg("ip 地址或子网掩码设置错误，格式类似1.1.1.1",-1);
            exit;
        }
    }

    for ($i=0;$i<count($ipdeniesegtest);$i++){

        if(!(filter_var($ipdeniesegtest[$i], FILTER_VALIDATE_IP) &&(filter_var($ipdeniesegmasktest[$i], FILTER_VALIDATE_IP))))
        {
            ShowMsg("ip 地址或子网掩码设置错误，格式类似1.1.1.1",-1);
            exit;
        }
    ...
    isreceipt='$isreceipt',
    ipallowseg='$ipallowseg',
    ipallowsegmask=\"$ipallowsegmask\",
    ipdenieseg=\"$ipdenieseg\",
    ipdeniesegmask=\"$ipdeniesegmask\",
     issend='$issend',
     sortrank='$sortrank',
     typename='$typename',
     typedir='$typedir',
