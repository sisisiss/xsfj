...
if($dopost!='save')
{
    ...
    require_once(DEDEADMIN."/inc/inc_moban_options.php");
    ClearMyAddon();
...
else if($dopost=='save')
{
    ...
    $deceiptuserids = empty($usersids)? "" : $usersids;
    $isdeceipt = 0;
    $deceiptedids = "";
    ...
    if(!empty($dede_addonfields))
    {
        ...
        if(is_array($addonfields))
        {
            foreach($addonfields as $v)
            {
                ...
                {
                    if(!isset(${$vs[0]})) ${$vs[0]} = '';
                    ${$vs[0]} = GetFieldValueA(${$vs[0]},$vs[1],$argetselectioncID);
                }
                $inadd_f .= ','.$vs[0];
                $inadd_v .= " ,'".${$vs[0]}."' ";
            }
    ...
    color,writer,source,litpic,pubdate,senddate,mid,voteid,notpost,description,keywords,filename,dutyadmin,weight,deceiptuserids,isdeceipt,deceiptedids)
    VALUES ('$arcID','$typeid','$typeid2','$sortrank','$flag','$ismake','$channelid','$arcrank','$click','$money',
    '$title','$shorttitle','$color','$writer','$source','$litpic','$pubdate','$senddate',
    '$adminid','$voteid','$notpost','$description','$keywords','$filename','$adminid','$weight','$deceiptuserids','$isdeceipt','$deceiptedids');";
