...
function ShowCatMap(e, obj, cid, targetId, oldvalue)
{
	...
	ChangeFullDiv('show',500);
}
//显示栏目Map地图
function ShowUsers(e, obj, cid, targetId, oldvalue)
{
    fixupPos = true;
    LoadQuickDiv(e, 'archives_do.php?dopost=getUsers&targetid='+targetId+'&channelid='+cid+'&oldvalue='+oldvalue+'&rnd='+Math.random(), 'getCatMap', '700px', '500px');
    ChangeFullDiv('show',500);
}
...
function getSelUser(targetId)
{
    var selBox = document.quicksel.seluserid;
    var targetObj = $Obj(targetId);
    var selvalue = '';
    var selname = '';

    //副栏目（多选）
    if(targetId=='typeidqs')
    {
        for(var i=0; i< selBox.length; i++)
        {
            if(selBox[i].checked) {
                ...
                selvalue += (selvalue=='' ? selBox[i].value : ','+selBox[i].value);
                selname += (selname=='' ? selBox[i].nextSibling.nextSibling.nextSibling.innerHTML : ',' + selBox[i].nextSibling.nextSibling.nextSibling.innerHTML);
            }
        }
        if(targetObj) {
        	targetObj.value = selname;
            $Obj("usersids").value=selvalue;
        }
