...
if($dopost=='saveedit')
{
    ...
    if($id!=1){
        $query = "UPDATE `#@__admin` SET uname='$uname',department='$department',usertype='$usertype',tname='$tname',email='$email',typeid='$typeid' $pwd WHERE id='$id'";
    }else{
        $query = "UPDATE `#@__admin` SET uname='$uname',department='$department',tname='$tname',email='$email',typeid='$typeid' $pwd WHERE id='$id'";
    }
...
$dsql->SetQuery("SELECT * FROM `#@__sys_enum` WHERE  egroup='zzjg'");
$dsql->Execute('dtp');
while($nrow = $dsql->GetObject('dtp'))
{

   $dptOptions .= "<option value='{$nrow->ename}' class='stype'".(in_array($nrow->id, $typeids) ? ' selected' : '').">{$nrow->ename}</option>\r\n";
