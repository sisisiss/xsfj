...
//$t1 = ExecTime();$tid = (isset($tid) && is_numeric($tid) ? $tid : 0);

if( isset($date) && strtotime($date) ){

    $date1=GetOnDutyDate(strtotime($date));
    $date2=date("Y年n月",$date1);
    $cinfo=$dsql->GetOne("SELECT typeid FROM #@__archives where title like '$date2'");
    $tid=$cinfo["typeid"];
    if(!$tid){
